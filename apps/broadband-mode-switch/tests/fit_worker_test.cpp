...
void test_snapshot_worker_and_candidate_visibility() {
  ...
  app::FitWorker worker;
  app::FitWorker::Request request;
  ...
  app::FitWorker::Event event;
  std::shared_ptr<app::Mlp> candidate;
  ...
  for (;;) {
    ...
    while (worker.poll(event)) {
      if (event.kind == app::FitWorker::EventKind::kProgress) {
        ++progress_count;
        expect(!event.candidate, "progress never exposes a candidate model");
      } else if (event.kind == app::FitWorker::EventKind::kSucceeded) {
