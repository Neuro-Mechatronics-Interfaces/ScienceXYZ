...
1. generates ``device-config.json`` / ``task-profile.json`` / ``provenance.json``
   ...
   ``scifi2_hub_manager.calibration_task.prepare_session``; the profile is
...
2. prints the exact ``synapsectl start`` line for the operator to run, and gates
   ...
   ``scifi2-hub-manager`` reports ``Running: True``;
...
from scifi2_hub_manager.calibration_task import load_profile, prepare_session

# Repository root is four levels up from apps/scifi2-hub-manager/client.
_REPO_ROOT = Path(__file__).resolve().parents[3]
_CLIENT_DIR = Path(__file__).resolve().parent
_APP_NAME = "scifi2-hub-manager"
