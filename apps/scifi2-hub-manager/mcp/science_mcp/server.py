"""Stdio MCP server for the SciFi-2 scifi2-hub-manager kApplication.
...
APP_NAME = "scifi2-hub-manager"

mcp = _Server(
    "science-mcp",
    instructions=(
        "Read-only device introspection and offline diagnostics for the SciFi-2 "
        "scifi2-hub-manager kApplication. This server never runs synapsectl "
...
def _client_module(name: str):
    try:
        return __import__(f"scifi2_hub_manager.{name}", fromlist=[name])
...
def device_info_from_capture(info_capture_path: str) -> dict[str, Any]:
    ...
    text file; this reports whether the ``scifi2-hub-manager`` App shows
...
def _import_analyze():
    try:
        ...
        raise RuntimeError(
            ...
            "recording extra: pip install -e 'apps/scifi2-hub-manager/client[recording]'"
        ) from exc
