...
using scifi2_hub::control::ActiveTarget;
using scifi2_hub::control::ControlCommandQueue;
using scifi2_hub::control::ControlRequest;
...
ControlCommand command(const char* request_id, std::uint32_t label) {
  ...
  result.set_command(stateful_decode_and_sync::v1::COMMAND_SELECT_LABEL);
...
void test_prepare_capture_is_one_atomic_transition() {
  ...
  const auto result = scifi2_hub::control::prepare_capture(target, 1, 3, true, 2, 4);
  ...
  const auto invalid = scifi2_hub::control::prepare_capture(target, 2, 0, false, 2, 4);
...
void test_selection_rejects_capture_and_invalid_targets_without_mutation() {
  ...
  const ActiveTarget before = target;
  const auto collection = scifi2_hub::control::select_collection(target, 1, 2, 4);
  expect(!static_cast<bool>(collection) &&
             collection.code == stateful_decode_and_sync::v1::ERROR_CAPTURE_ENABLED,
  ...
  const auto label = scifi2_hub::control::select_label(target, 4, 4);
  ...
  expect(static_cast<bool>(scifi2_hub::control::select_collection(target, 1, 2, 4)),
