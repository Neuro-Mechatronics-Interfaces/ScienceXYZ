...
void test_off_diagonal_band_limit_controls_dimension() {
  scifi2_hub::MpfFeaturizer::Config config;
  ...
  scifi2_hub::MpfFeaturizer featurizer(config);
  ...
  scifi2_hub::MpfFeaturizer diagonal_only(config);
...
void test_decimation_plan_preserves_window_and_stride_clock() {
  expect(scifi2_hub::next_power_of_two(250) == 256 && scifi2_hub::next_power_of_two(256) == 256 &&
             scifi2_hub::next_power_of_two(500) == 512,
         "FFT sizing rounds up dynamically without expanding exact powers of two");
  const auto plan =
      scifi2_hub::make_feature_decimation_plan(20000.0, 1000.0, 1.25, 4000, 400);
  ...
  scifi2_hub::MpfFeaturizer::Config requested_bands;
  requested_bands.num_channels = 1;
  requested_bands.sample_rate_hz = plan.feature_sample_rate_hz;
  requested_bands.stft_size = scifi2_hub::next_power_of_two(4000 / plan.factor);
  ...
  scifi2_hub::MpfFeaturizer requested_featurizer(requested_bands);
...
void test_batches_are_windowed_off_thread() {
  scifi2_hub::MpfFeaturizer::Config featurizer_config;
  featurizer_config.num_channels = 1;
  featurizer_config.stft_size = 4;
  featurizer_config.stft_hop = 2;
  featurizer_config.num_bands = 2;

  scifi2_hub::FeatureWorker worker;
  scifi2_hub::FeatureWorker::Config config;
  config.channel_map = {0};
  config.window_samples = 8;
  config.stride_samples = 2;
  config.featurizer = std::make_shared<scifi2_hub::MpfFeaturizer>(featurizer_config);
  ...
  scifi2_hub::FeatureWorker::SampleBatch batch;
  ...
  scifi2_hub::FeatureWorker::Result result;
...
void test_streaming_decimator_centers_metadata_and_averages() {
  ...
  scifi2_hub::FeatureDecimationPlan plan;
  plan.factor = 2;
  plan.group_delay_source_samples = 1;
  plan.coefficients = {0.0, 1.0, 0.0};
  scifi2_hub::StreamingDecimator decimator(plan);

  std::vector<std::uint64_t> sequences;
  std::vector<std::int32_t> values;
  scifi2_hub::StreamingDecimator::Sample out;
  ...
  scifi2_hub::FeatureDecimationPlan avg_plan;
  avg_plan.factor = 2;
  avg_plan.group_delay_source_samples = 1;
  avg_plan.coefficients = {1.0 / 3.0, 1.0 / 3.0, 1.0 / 3.0};
  scifi2_hub::StreamingDecimator averager(avg_plan);
...
void test_streaming_decimator_passthrough_when_factor_one() {
  scifi2_hub::FeatureDecimationPlan plan;  // factor defaults to 1
  scifi2_hub::StreamingDecimator decimator(plan);
  scifi2_hub::StreamingDecimator::Sample out;
