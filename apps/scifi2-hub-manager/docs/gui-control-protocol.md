...
| `set_exo_mode` | `mode` (`off`/`external`/`decode`) | Engage the optional on-device exo link. `off` (default) keeps the hand disarmed; `external` opens+arms it for `set_exo_pose`; `decode` lets the decoder drive it. Requires exo support enabled in config. |
| `set_exo_pose` | `joints` (1-6 `{joint,value}`, value signed `[-100,100]`) | Command a finger pose while in `external` mode. -100 extend, 0 rest, +100 flex; omitted joints hold. Requires exo firmware >= 0.6.4. |
...
<!-- graphviz:apps/scifi2-hub-manager/docs/gui-control-architecture.dot -->
![GraphViz diagram](gui-control-architecture.svg)
<!-- /graphviz:apps/scifi2-hub-manager/docs/gui-control-architecture.dot -->
