...
| `synapsectl apps build apps/scifi2-hub-manager` | Baseline build could not start for the same reason; no source compilation result was produced. |
...
- consumer `control`: typed `scifi2-hub-manager.v1.ControlCommand`, validated and bounded-queued for serial main-loop application;
...
```text
cmake -S apps/scifi2-hub-manager -B apps/scifi2-hub-manager/build/offline-tests -DBUILD_DEVICE_APP=OFF -DBUILD_TESTING=ON
cmake --build apps/scifi2-hub-manager/build/offline-tests
ctest --test-dir apps/scifi2-hub-manager/build/offline-tests --output-on-failure
```

The registered tests are `scifi2-hub-manager-control`,
`scifi2-hub-manager-protocol`, `scifi2-hub-manager-control-state`,
`scifi2-hub-manager-mlp`, and `scifi2-hub-manager-fit-worker`.
