...
struct TransitionResult {
  bool success = false;
  protocol::ErrorCode code = stateful_decode_and_sync::v1::ERROR_NONE;
  std::string field;
  std::string message;
...
inline TransitionResult success() { return {true, stateful_decode_and_sync::v1::ERROR_NONE, {}, {}}; }
...
inline TransitionResult validate_target(std::uint32_t collection_id, std::uint32_t label,
  ...
  if (collection_count == 0 || collection_id >= collection_count) {
    return failure(stateful_decode_and_sync::v1::ERROR_OUT_OF_RANGE, "collection_id",
                   "collection is outside the configured bank");
  }
  if (labels_per_collection == 0 || label >= labels_per_collection) {
    return failure(stateful_decode_and_sync::v1::ERROR_OUT_OF_RANGE, "label",
...
inline TransitionResult select_collection(ActiveTarget& current, std::uint32_t collection_id,
  ...
  if (current.capture_enabled) {
    return failure(stateful_decode_and_sync::v1::ERROR_CAPTURE_ENABLED, "capture_enabled",
