...
// SciFi-2 Hub Manager: the configured on-device acquisition/control App.
//
// Node graph:
//   kBroadbandSource(id=1) -> kApplication(id=2, "scifi2-hub-manager")
//
// Consumer taps:
...
//
// Producer taps:
//   broadband_out    BroadbandFrame            decimated real or synthetic
...
// requests, then return fast. The main loop anti-aliases/decimates raw frames
// and transfers value-owned sample batches to FeatureWorker. FeatureWorker
// window-aligns samples and sends completed windows to its FIFO compute
...
// training off-thread.
class SciFi2HubManagerApp : public synapse::App {
 public:
  SciFi2HubManagerApp() = default;
  ~SciFi2HubManagerApp() override;
 ...
 private:
  ...
  struct AppConfig {
    ...
    std::vector<exo::JointPose> exo_class_poses;
    // Minimum softmax probability for the winning class to drive a pose in
    // DECODE mode; below this the hand holds its last pose.
    float exo_decode_min_confidence = 0.6f;
  };
  ...
  // drive the exo. Called from drain_feature_results() alongside publish_class.
  void drive_exo_from_decode(const std::vector<float>& probs);
  void fill_exo_status(stateful_decode_and_sync::v1::ExoStatus* status) const;
  ...
  // Optional exo link. Constructed only when cfg_.exo_enabled; null otherwise.
  // The current engagement mode is main-thread owned (mutated only in
  // apply_set_exo_mode) and read for the state snapshot and the decode hook.
  std::unique_ptr<exo::ExoLinkWorker> exo_worker_;
  stateful_decode_and_sync::v1::ExoMode exo_mode_ =
      stateful_decode_and_sync::v1::EXO_MODE_OFF;
