...
int main(int argc, char** argv) {
  ...
  scifi2_hub::recording::TaskTimeline timeline;
  scifi2_hub::wireless::AffineClockEstimator clock;  // auxiliary-source clock history
  scifi2_hub::recording::TaskTimelineAdapter adapter(timeline, options.reference_source_id,
                                              &host_steady_now_ns);

  scifi2_hub::recording::Hdf5RecordSink sink(options.output_path);
  scifi2_hub::recording::TaskRecorderHdf5Writer writer(sink);
  ...
  while (!g_stop_requested.load() && !failed) {
    ...
    for (bool reference : {false, true}) {
      ...
      std::vector<scifi2_hub::recording::RawTapMessage> batch;
