# scifi2_hub_manager App — Design (plan before code)

Status: **design locked, implementation pending.** A new on-device Synapse App
under `apps/scifi2-hub-manager/`, modelled on `apps/synapse-example-app`
...
kBroadbandSource(id=1, peripheral_id=200, 20 kHz, 16-bit, 32 ch)
        │  (connection 1 -> 2)
        ▼
kApplication(id=2, name="scifi2_hub_manager")
        │
